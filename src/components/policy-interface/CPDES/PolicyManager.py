...
class FTBEventListener():
    ...
    def startFTBListner(self, sHandle):
...
#            print 'Inserting %s, %s into the EventQueue' % (eventAttrib.name,
#                                                            eventAttrib.eventType)
            self.eventQueue.put([cEvent, eventAttrib])
            

class PolicyManager(multiprocessing.Process):
    def __init__(self, eq):
        multiprocessing.Process.__init__(self, target = self.eventManager, \
                                         args = (eq, ))
        self.eventQueue = eq 
        self.daemon = True
        self.start()

    def eventManager(self, eventQueue):
        while True:
            cEvent, eventAttrib = eventQueue.get()
            print 'Processing %s, id: %s' % (eventAttrib.name, eventAttrib.id)
            self.processEvent(cEvent, eventAttrib)
        
    def processEvent(self, cEvent, eventAttrib):
...
if __name__ == '__main__':
    ...
    pm = PolicyManager(eventQueue)

    FTBEventListener(eventQueue)
