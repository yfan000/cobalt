...
class ScriptManager(Cobalt.Component.Component, Cobalt.Data.DataSet):
    '''The ScriptManager supports the running of scripts on a BG machine'''
    __implementation__ = 'scriptm'
    __name__ = 'script-manager'
...
if __name__ == '__main__':
    ...
    try:
        ...
        print "%s\nUsage:\nscriptm.py [-d] [-C config file] [-D <pidfile>]" % (msg)
        raise SystemExit, 1
    try:
    ...
    if len([item for item in opts if item[0] == '-d']):
        ...
        dlevel=logging.INFO
    Cobalt.Logging.setup_logging('scriptm', level=dlevel)
    s = ScriptManager({'configfile':'/etc/cobalt.conf', 'daemon':daemon})
    s.serve_forever()
