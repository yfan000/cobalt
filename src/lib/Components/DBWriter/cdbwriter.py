    ...
    def iterate(self):
        ...
        while self.msg_queue and self.connected:
            ...
            try:
                ...
                if (self.max_queued != None) and(len(self.msg_queue) >= self.max_queued):
                    self.overflow = True
        ...
        try:
            for table_name in table_names:
                ...
                if table_name in ['RESERVATION_EVENTS', 'JOB_EVENTS', 'JOB_COBALT_STATES']:
                    self.daos[table_name] = StateTableData(self.db, schema, table_name)
                elif table_name == 'RESERVATION_DATA':
                    self.daos[table_name] = ResDataData(self.db, schema, table_name)
                elif table_name == 'JOB_DATA':
                    self.daos[table_name] = JobDataData(self.db, schema, table_name)
                elif table_name == 'JOB_DEPS':
                    self.daos[table_name] = JobDepsData(self.db, schema, table_name)
                elif table_name == 'JOB_PROG':
                    self.daos[table_name] = JobProgData(self.db, schema, table_name)
                elif table_name in no_pk_tables:
                    self.daos[table_name] = no_pk_dao(self.db, schema, table_name)
                else:
                    self.daos[table_name] = db2util.dao(self.db, schema, table_name)
        except:
        ...
        if logMsg.item.users:
            ...
            if user_list[0] != '':
                for user in user_list:
                    res_users_record = self.daos['RESERVATION_USERS'].table.getRecord({'RES_DATA_ID': res_data_id,
        ...
        for fieldName in job_prog_msg.__dict__.keys():
            if fieldName in ['envs', 'location',
                ...
                if fieldName not in ['jobid', 'cobalt_state']:
                    job_prog_record.v.__setattr__(fieldName.upper(), job_prog_msg.__getattribute__(fieldName))
